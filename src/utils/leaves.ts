...
const speed = 40; // lower number for faster
const leaves = 3; // number of leaves falling at a time
const untidy = 4; // how often do you want the leaves tidied up (high number is less often)
...
let deeex = 0;
let has_focus = true;
const plow = document.createElement('img');
...
const leafy: (HTMLImageElement | false)[] = [];
const leaf_image: string[] = [];
...
function addLoadEvent(funky: () => void) {
...
function start_leaf(whyp: number) {
  starty++;
  offset++;
  let f; let
    size;
...
function start_fall(i: number, whyp: number) {
  const size = 72 - Math.floor(36 * Math.random());
  dx[i] = Math.random();
...
function autumn_leaves() {
  ...
  let c = 0;
  for (i = 0; i < starty; i++) {
    const leafyElement = leafy[i];
    if (leafyElement && le[i] != 'tidying') {
      if (yp[i] > shigh || xp[i] > swide || xp[i] < -leafyElement.width) {
        if (offset > 0) offset--;
        boddie.removeChild(leafyElement);
        leafy[i] = false;
      } else if (yp[i] + untidy * offset / leaves < shigh - leafyElement.height / 2) {
        yp[i] += dy[i];
...
function plough(x: number) {
  let i;
  let p;
  plow.style.left = `${x}px`;
  for (i = 0; i < starty; i++) {
    const leafyElement = leafy[i]
    if (leafyElement && le[i] != 'falling') {
      p = xp[i] + leafyElement.width + am[i] * Math.sin(dx[i]) - dy[i];
      if (p < 0) {
        boddie.removeChild(leafyElement);
        leafy[i] = false;
      } else if (p > x && p < x + 3) {
      ...
      } else if (p > x + 144 && yp[i] < shigh - leafyElement.height / 2) {
        yp[i] += dy[i];
