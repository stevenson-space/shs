  ...
  <div>
    ...
    <card class="top-card">
      <div class="top-card-inner">
        ...
        <div class="edit-note">
          Course names are editable and can automatically set credit level from the name • Full year course is different from a one semester course • Matches Infinite Campus Layout
        </div>
      </div>
    </card>
        ...
        <div class="semester-grid">
          ...
          >
            ...
            <div class="footer-actions">
              <button
                class="sketch-btn"
                :disabled="semester.courses.length >= getMaxCoursesPerSemester(year)"
              ...
              <button
                class="sketch-btn"
                :disabled="isFullYearDisabled(year, sIdx)"
                @click="addFullYear(yIdx, sIdx)"
              >
...
const REGULAR_MAX_COURSES_PER_SEMESTER = 7;
const SUMMER_MAX_COURSES_PER_SESSION = 1;
...
export default defineComponent({
  ...
  computed: {
    ...
    {
      return this.years.filter((y) => !y.isSummer).length;
    },
    ...
    {
      const count = this.schoolYearCount;

      if (count >= 4)
      {
        return 'All Years Added';
      }

      return 'Add ' + YEAR_LABELS[count];
    },

    canAddSummer(): boolean
    {
      if (this.summerCount >= 5)
      {
        return false;
      }
      ...
      return !!last && !last.isSummer;
    },

    nextSummerLabel(): string
    {
      const idx = this.summerCount;
      return 'Add ' + (SUMMER_LABELS[idx] || 'Post-Senior Summer');
    },
  },
  ...
  methods: {
    ...
    {
      return new Course(name, linkedId);
    },

    isSemesterAllowedToBeBlank(year: YearBlock, semesterIdx: number): boolean
    {
      return !year.isSummer && semesterIdx === 1;
    },

    getMaxCoursesPerSemester(year: YearBlock): number
    {
      if (year.isSummer)
      {
        return SUMMER_MAX_COURSES_PER_SESSION;
      }

      return REGULAR_MAX_COURSES_PER_SEMESTER;
    },
    ...
    normalizeGrade(value: unknown): number
    {
      ...
      {
        if (value >= 0 && value < this.gradeLabels.length)
        {
          return value;
        }
      ...
      if (typeof value === 'string')
      {
        ...
        const idx = this.gradeLabels.findIndex((g) => g === cleaned);
        return idx >= 0 ? idx : 0;
      }
    ...
    normalizeLevel(value: unknown): number
    {
      ...
      if (typeof value === 'string')
      {
        ...
        if (cleaned.includes('honors') || cleaned.includes('honor'))
        {
          return 2;
        }
      }
    ...
    detectLevelFromCourseName(name: string): number | null
    {
      ...
      if (/\bhonors\b/.test(lowered) || /\bhonor\b/.test(lowered) || /\bAP\b/.test(trimmed))
      {
        return 2;
      }
    ...
    loadSavedData()
    {
      try
      {
        ...
        this.years = parsed.map((year: any) =>
        {
          ...
          const fallbackLinkedId = this.createId();
          const fallbackSemesters = isSummer
            ? [{ courses: [] }, { courses: [] }]
            : [
                { courses: [this.createCourse('Course 1', fallbackLinkedId)] },
                { courses: [this.createCourse('Course 1', fallbackLinkedId)] },
              ];
                        ...
                        ({
                          instanceId:
                            typeof course.instanceId === 'string'
                              ...
                              : this.createId(),
                          linkedId:
                            typeof course.linkedId === 'string'
                              ? course.linkedId
                              : null,
                          name: typeof course.name === 'string' ? course.name : '',
                          grade: this.normalizeGrade(course.grade),
                          ...
                          weight:
                            course.weight === 1.5 || course.weight === 1.0
                              ...
                              : 1.0,
                          unweightedGPA:
                            typeof course.unweightedGPA === 'number'
                              ? course.unweightedGPA
                              : 4.0,
                          weightedGPA:
                            typeof course.weightedGPA === 'number'
                              ? course.weightedGPA
                              : 4.0,
                          finalGrade:
                            typeof course.finalGrade === 'string'
                              ? course.finalGrade
                              : 'A',
                        } as Course))
                      : [],
          ...
          return {
            label: typeof year.label === 'string' ? year.label : 'Untitled Year',
            isSummer,
            semesters,
          } as YearBlock;
        });

        this.ensureValidSemesterStructure();
        this.calculateAll();
      }
      catch (error)
      {
    ...
    ensureValidSemesterStructure()
    {
      this.years.forEach((year) =>
      {
        if (year.isSummer)
        {
          while (year.semesters.length < 2)
          {
            year.semesters.push({ courses: [] });
          }

          if (year.semesters.length > 2)
          {
            year.semesters = year.semesters.slice(0, 2);
          }
        }
        else
        {
          ...
          if (year.semesters[0].courses.length === 0 && year.semesters[1].courses.length === 0)
          {
            const linkedId = this.createId();
            year.semesters[0].courses.push(this.createCourse('Course 1', linkedId));
            year.semesters[1].courses.push(this.createCourse('Course 1', linkedId));
          }
        }
      });
    },
    ...
    addYear()
    {
      ...
      this.years.push(this.createRegularYear(YEAR_LABELS[count]));
      this.calculateAll();
    },

    addSummer()
    {
      if (!this.canAddSummer)
      {
        return;
      }

      const idx = this.summerCount;
      this.years.push(this.createSummerYear(SUMMER_LABELS[idx] || 'Post-Senior Summer'));
      this.calculateAll();
    },
    ...
    addCourse(yIdx: number, sIdx: number)
    {
      ...
      const maxCourses = this.getMaxCoursesPerSemester(year);

      if (semester.courses.length >= maxCourses)
      {
        return;
      }
    ...
    isFullYearDisabled(year: YearBlock, semesterIdx: number): boolean
    {
      const semester = year.semesters[semesterIdx];
      ...
      if (year.isSummer)
      {
        const otherSemesterIdx = semesterIdx === 0 ? 1 : 0;
        ...
        return semester.courses.length > 0 || otherSemester.courses.length > 0;
      }
      ...
      return (
        year.semesters[0].courses.length >= maxCourses ||
        year.semesters[1].courses.length >= maxCourses
      );
    },

    renumberSemesterCourses(year: YearBlock, semesterIdx: number)
    {
      const semester = year.semesters[semesterIdx];
      ...
      {
        if (course.name.trim().match(/^Course \d+$/i))
        {
    ...
    addFullYear(yIdx: number, clickedSemesterIdx: number)
    {
      const year = this.years[yIdx];

      if (!year || this.isFullYearDisabled(year, clickedSemesterIdx))
      {
        return;
      }
      ...
      if (year.isSummer)
      {
        ...
        const firstCourse = this.createCourse('Course 1', linkedId);
        const secondCourse = this.createCourse('Course 1', linkedId);

        firstSemester.courses.push(firstCourse);
        secondSemester.courses.push(secondCourse);

        this.calculateAll();
        return;
      }

      const otherSemesterIdx = clickedSemesterIdx === 0 ? 1 : 0;
      const clickedSemester = year.semesters[clickedSemesterIdx];
      const otherSemester = year.semesters[otherSemesterIdx];

      if (!clickedSemester || !otherSemester)
      {
        return;
      }
      ...
      if (
        clickedSemester.courses.length >= maxCourses ||
        otherSemester.courses.length >= maxCourses
      )
      {
        return;
      }

      const insertIndex = clickedSemester.courses.length;
      const defaultName = `Course ${insertIndex + 1}`;

      const clickedCourse = this.createCourse(defaultName, linkedId);
      const otherCourse = this.createCourse(defaultName, linkedId);

      clickedSemester.courses.splice(insertIndex, 0, clickedCourse);
      otherSemester.courses.splice(
        Math.min(insertIndex, otherSemester.courses.length),
        0,
        otherCourse
      );

      this.renumberSemesterCourses(year, clickedSemesterIdx);
      this.renumberSemesterCourses(year, otherSemesterIdx);
      this.calculateAll();
    },
    ...
    removeCourse(yIdx: number, sIdx: number, course: Course)
    {
      ...
      if (!course.linkedId)
      {
        this.renumberSemesterCourses(year, sIdx);
      }
